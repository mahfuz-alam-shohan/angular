...
@Component({
    selector: "app-cheif-patron",
    templateUrl: "./cheif-patron.component.html",
    styleUrls: ["./cheif-patron.component.scss"],
})
export class CheifPatronComponent {
        ...
        forkJoin([this.appService.getGBMessages()]).subscribe(
            (response: any) => {
                ...
                for (let i = 0; i < this.GBMessages.length; i++) {
                    if (this.GBMessages[i].GBDesignationName === "Chief Patron") {
                ...
                if (targetArray) {
                    console.log(targetArray,"Cheif Patron");
                } else {
                    console.log(
